...
SGE::Navigation::Navigation()
    : proj(new QMatrix4x4), view(new QMatrix4x4), scene(nullptr),
      func(new QOpenGLFunctions)
{
}


SGE::Navigation::Navigation(std::shared_ptr<QMatrix4x4> proj,
                            std::shared_ptr<QMatrix4x4> view,
                            std::shared_ptr<Scene> scene, const QRect &VP)
    : proj(proj), view(view), scene(scene), VP(VP), func(new QOpenGLFunctions)
{
}


void SGE::Navigation::initNavigation() { func->initializeOpenGLFunctions(); }
...
void SGE::Navigation::keyPress(QKeyEvent *ev)
{
    if (scene->selectionObjectExist())
    {
        if (ev->key() == Qt::Key_G)
        {
            ...
            diff.setY(VP.height() - diff.y() - static_cast<qint32>(origin.y()));
...
{
    GLuint index;
    func->glReadPixels(mouse.x(), VP.height() - mouse.y(), 1, 1,
...
void SGE::Navigation::moveObj()
{
    ...
    crntObjPos = crntObjPos.unproject((*view) * (*crntObjModel), *proj, VP);
